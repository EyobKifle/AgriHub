...
// Load current user preferences and profile data from DB
$data = [];
...
")) {
    ...
    if ($row = $res->fetch_assoc()) {
        $data = $row;
        $name = $data['name']; // Override session name with DB value
        $email = $data['email'];
    }
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $avatar_path = $data['avatar_url'] ?? null; // Keep existing if no new upload
        ...
        $stmt = $conn->prepare('INSERT INTO user_profiles (user_id, bio, farm_size_hectares, specialization, experience_years, business_name, business_address, language_preference, pref_theme, pref_email_notifications) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE bio = VALUES(bio), farm_size_hectares = VALUES(farm_size_hectares), specialization = VALUES(specialization), experience_years = VALUES(experience_years), business_name = VALUES(business_name), business_address = VALUES(business_address), language_preference = VALUES(language_preference), pref_theme = VALUES(pref_theme), pref_email_notifications = VALUES(pref_email_notifications)');
        ...
        $conn->commit();
        header('Location: User-Account.php?saved=1');
        exit();
...
<body>
   ...
   <div class="dashboard-container">
      ...
      <main class="main-content">
         ...
         <form class="profile-layout" method="post" enctype="multipart/form-data">
            ...
            <div class="card profile-picture-card">
               ...
               <div class="profile-picture-preview">
                  <img src="<?php echo !empty($data['avatar_url']) ? '../' . e($data['avatar_url']) : 'https://placehold.co/150x150/2a9d8f/FFF?text=' . e($initial); ?>" alt="Current profile picture" id="profile-image-preview">
               </div>
            ...
            <div class="card profile-details-card">
               ...
               <div class="form-group">
                  <label for="bio" data-i18n-key="user.profile.details.about">About Me / My Farm</label>
                  <textarea id="bio" name="bio" rows="4" placeholder="Describe your farm, the crops you grow..."><?php echo e($data['bio'] ?? ''); ?></textarea>
               </div>
               ...
               <div class="form-actions" style="margin-top: 2rem;">
                  <button type="submit" class="btn btn-primary" data-i18n-key="user.profile.details.save">Save All Changes</button>
               </div>
            </div>
         </form>
      </main>
   ...
   <script>
      // Simple script for avatar preview
